<template>
	<view class="u-swiper-wrap u-padding-25" :style="{
		...
		<swiper :current="elCurrent" @change="change" @animationfinish="animationfinish" :interval="interval" :circular="circular" :duration="duration" :autoplay="autoplay"
		 ...
		 :style="{
			...
			<swiper-item class="u-swiper-item" v-for="(item, index) in list" :key="index">
				<view class="u-list-image-wrap u-flex bg-white" @tap.stop.prevent="listClick(index)" :class="[uCurrent != index ? 'u-list-scale' : '']" :style="{
						borderRadius: `${borderRadius}rpx`,
						transform: effect3d && uCurrent != index ? 'scaleY(0.9)' : 'scaleY(1)',
						margin: effect3d && uCurrent != index ? '0 20rpx' : 0,
					}">
					<image class="u-swiper-image u-margin-left-25 u-margin-right-25" v-for="(obj,num) in item[0]" :key="num" :src="obj[name] || obj" :mode="imgMode" @click="toDetail(item)"></image>
	...
	export default {
		...
		methods: {
			toDetail(item) {
				console.log(item);
				uni.navigateTo({
					url: '../../subPackages/article/articleDetail/articleDetail'
				})
			},
			listClick(index) {
