<template>
	<view class="u-notice-bar" :style="{
		...
		<swiper :disable-touch="disableTouch" @change="change" :autoplay="autoplay && playState == 'play'"
			:vertical="vertical" circular :interval="duration" :class="'u-swiper ' + (displayLine==2?'two-line-height':'')">
			<swiper-item v-for="(item, index) in list" :key="index" class="u-swiper-item">
				<view>
					...
					<view class="u-news-item u-line-1" :style="[textStyle]" @tap="click(index)" :class="['u-type-' + type]" v-if="displayLine==2">
						{{ (index + 1) >= list.length ? list[0] : list[index+1] }}
					</view>
				</view>
			</swiper-item>
		</swiper>
