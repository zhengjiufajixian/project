<template>
	<view>
		...
		<view class="u-padding-left-30 u-padding-right-30 bg-white">
			...
			<view>
				<view class="u-flex u-col-center u-row-between line-height-3" v-for="(item,index) in orderDetail.maintainRemark"
				 :key="index">
					<view class="">
			...
			<view class="u-flex u-col-center u-row-between line-height-3">
				...
				<view>
					<text class="color-4A4A4A u-font-26">合计： <text class="color-FF2828">￥ {{orderDetail.orderMoney}}</text></text>
				</view>
			</view>
		</view>
		...
		<view class="u-padding-left-30 u-padding-right-30 bg-white">
			...
			<view class="u-flex line-height-3 u-font-26">
				...
				<view class="u-flex-6">
					<text class="color-333333" v-show="orderDetail.carModel">车辆类型：<text class="color-666666">{{orderDetail.carModel}}</text></text>
				</view>
			</view>
		</view>
		...
		<movable-area v-if="orderDetail.orderStatus != 'INIT' && orderDetail.orderStatus != 'WAITCONFIRM' && orderDetail.orderStatus != 'PAYOFF'">
			<movable-view direction="vertical" y="100">
				<view class="u-padding-left-30 u-padding-right-40">
					<navigator class="u-flex" :url="'addImage/addImage?data=' + JSON.stringify(orderDetail)" hover-class="none">
						<view class="u-font-0">
		...
		<view
			v-if="orderDetail.orderStatus"
			class="u-flex u-margin-top-40 u-padding-left-28 u-padding-right-28 u-row-between u-col-center u-text-center">
			<view class="u-margin-left-28 u-margin-right-28 u-flex-6 cancel-btn" @click="handleCall" v-if="orderDetail != 'SUCCESS'  && orderDetail != 'REFUND' ">联系客户</view>
			<view class="u-margin-left-28 u-margin-right-28 u-flex-6 confirm-btn" @click="getBtnClick">{{getBtnText(orderDetail)}}
			</view>
		</view>
		...
		methods: {
			...
			getBtnClick() {
				...
				if (val.orderStatus == "WAITCONFIRM") {
					uni.navigateTo({
						url: '/subPackages/bookingOrder/orderDetail/payBookingOrder/payBookingOrder?orderId=' + val.id
					})
				}
				...
				if (val.orderStatus == "EXECUTING") {
					...
					this.$u.api.setOrderStatus({
						orderId: val.id,
						status: status,
						before: JSON.stringify(this.imgList1),
						after: JSON.stringify(this.imgList2)
					}).then(res => {
						console.log(res);
						if(res.code == 200){
							uni.showToast({
